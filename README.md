...
- A NUT server that permits read-only `LIST VAR` queries from this PC, usually on TCP port 3493.
...
2. In **Connection**, enter your NUT server, port and UPS name. If the server returns `ACCESS-DENIED`, allow this PC's address in the UPS server's client list.
...
The initial state is monitoring-only. No UPS address, personal settings or log files are included in the release archive. The language selector changes the interface without restarting monitoring.
